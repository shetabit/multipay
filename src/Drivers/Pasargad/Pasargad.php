...
class Pasargad extends Driver
{
    ...
    public function pay(): RedirectionForm
    {
        ...
        $statusCode = $paymentResult['resultCode'] ?? null;
        ...
        if (empty($paymentResult['data']['url'])) {
    ...
    public function verify(): ReceiptInterface
    {
        ...
        $statusCode = $invoiceInquiry['resultCode'] ?? null;
        ...
        $invoiceInquiryStatus = $invoiceDetails['status'] ?? null;

        if ($invoiceInquiryStatus !== 0) {
            $errorMessage = $this->translateStatus($invoiceInquiryStatus);

            throw new InvalidPaymentException($errorMessage, (int) $invoiceInquiryStatus);
        }
     ...
     * @return string
     */
    protected function translateStatus(int|string|null $status, ?string $fallbackMessage = null): string
    {
        $translations = [
            0 => 'تراکنش موفق است',
            1 => 'ناموفق',
    ...
    protected function getToken(): string
    {
        ...
        $statusCode = $result['resultCode'] ?? null;

        if ($statusCode !== 0) {
            $errorMessage = $this->translateStatus($statusCode, $result['resultMsg'] ?? 'Invalid Authentication');
